...
function emit(cat, key, value) {
  send({ cat, key, value: value === void 0 ? null : value });
}
...
try {
  ...
  try {
    ...
    try {
      imports = Module.enumerateImports(main.name);
    } catch (e2) {
    }
  }
  emit(
...
try {
  ...
  const seenMods = [];
  Process.enumerateModules().slice(0, 40).forEach(function(m2) {
    let syms = [];
    try {
      syms = m2.enumerateSymbols();
    } catch (e) {
      return;
    }
    const hits = syms.filter(function(s) {
      return TLS.test(s.name);
    }).map(function(s) {
      return s.name;
    });
    if (hits.length) {
      seenMods.push(m2.name);
      emit("TLSSYM", m2.name, hits.slice(0, 12).join(", "));
    }
  });
  if (!seenMods.length) emit("NOTE", "TLS symbols", "none exported by any module");
} catch (e) {
  emit("NOTE", "TLS scan", String(e));
}
if (!frida_objc_bridge_default.available) {
  emit("ERROR", "objc", "runtime not available");
} else {
  let callerOf = function(ctx) {
    try {
  ...
  }, watch = function(name, re) {
    const cls = frida_objc_bridge_default.classes[name];
    if (!cls) {
      emit("ABSENT", name, null);
      return;
    }
    let methods;
    try {
    ...
    methods.forEach(function(m2) {
      ...
      if (re && !re.test(m2)) return;
      try {
        Interceptor.attach(cls[m2].implementation, {
          onEnter: function() {
            const tag = name + " " + m2;
  ...
  WATCH_ALL.forEach(function(n) {
    watch(n, null);
  });
  Object.keys(WATCH_MATCH).forEach(function(n) {
    watch(n, WATCH_MATCH[n]);
  });
  let defHooks = 0;
  [
    "- objectForKey:",
    ...
    try {
      ...
      if (!m2) {
        emit("NOTE", "NSUserDefaults " + sel2, "selector not found");
        return;
      }
      Interceptor.attach(m2.implementation, {
        onEnter: function(args) {
          try {
            const k = new frida_objc_bridge_default.Object(args[2]).toString();
      ...
      emit("NOTE", "NSUserDefaults " + sel2, String(e));
    }
  });
  ["CFPreferencesCopyAppValue", "CFPreferencesCopyValue"].forEach(function(fn) {
    try {
      ...
      if (!p) {
        emit("NOTE", fn, "not exported");
        return;
      }
      Interceptor.attach(p, {
        onEnter: function(args) {
          try {
            const k = new frida_objc_bridge_default.Object(args[0]).toString();
            if (once("def " + k)) emit("DEFAULTS", k, callerOf(this));
          } catch (e) {
          }
        }
      ...
      emit("NOTE", fn, String(e));
    }
  });
  emit("NOTE", "defaults hooks attached", String(defHooks));
  ["- canOpenURL:", "- openURL:options:completionHandler:"].forEach(function(sel2) {
    try {
      const m2 = frida_objc_bridge_default.classes.UIApplication[sel2];
      if (!m2) {
        emit("NOTE", "UIApplication " + sel2, "selector not found");
        return;
      }
      Interceptor.attach(m2.implementation, {
        onEnter: function(args) {
          try {
            const u = new frida_objc_bridge_default.Object(args[2]).toString();
      ...
      emit("NOTE", "UIApplication " + sel2, String(e));
    }
  });
  const JB = new RegExp([
    "Cydia",
  ...
  [
    ...
    ["sysctl", "raw sysctl, the numeric form"]

...
// observe.js
var FLUSH_MS = 400;
var MAX_DISTINCT = 3e3;
var MAX_EVENTS = 2e6;
var HOT_MAX_MS = 9e3;
var TLS_ENABLED = 0;
var SYSCALL_ENABLED = 0;
var TOUCH_ENABLED = 0;
  ...
  self = Interceptor.attach(target, {
    ...
    onLeave: function(ret) {
      if (this._s && handlers.onLeave) {
        try {
          handlers.onLeave.call(this, ret);
        } catch (e) {
        }
      }
    }
...
function flush() {
  ...
  if (!_hotKilled && Date.now() - _t0 > HOT_MAX_MS) {
    _hotKilled = true;
    _hotListeners.forEach(function(l2) {
      try {
        l2.detach();
      } catch (e) {
      }
...
if (!frida_objc_bridge_default.available) {
  ...
  if (TLS_ENABLED) {
    ["SSLWrite", "SSL_write"].forEach(function(nm) {
      hotHook(
        Module.findGlobalExportByName(nm),
        400,
        "TLS write (" + nm + ")",
  ...
  if (TLS_ENABLED) {
    hotHook(
      Module.findGlobalExportByName("SSLRead"),
      400,
      "TLS read (SSLRead)",
      makeReadHandlers(true)
    );
    hotHook(
      Module.findGlobalExportByName("SSL_read"),
      400,
      "TLS read (SSL_read)",
  ...
  if (TOUCH_ENABLED) {
    try {
      ...
      if (m2) {
        hotHook(m2.implementation, 40, "scroll velocity reads", {
          onEnter: function() {
  ...
  if (TOUCH_ENABLED) {
    try {
      ...
      if (m2) {
        hotHook(m2.implementation, 30, "device-motion value samples", {
          onLeave: function(ret) {
            try {
              if (ret.isNull()) return;

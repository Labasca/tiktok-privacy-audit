...
// tools/phkit_probe.js
function post(stage, detail) {
  send({ stage, detail: String(detail) });
}
function requestFor(kind, url) {
  const R = frida_objc_bridge_default.classes.PHAssetCreationRequest;
  return kind === "image" ? R.creationRequestForAssetFromImageAtFileURL_(url) : R.creationRequestForAssetFromVideoAtFileURL_(url);
}
function urlFor(path) {
  return frida_objc_bridge_default.classes.NSURL.fileURLWithPath_(
    frida_objc_bridge_default.classes.NSString.stringWithUTF8String_(Memory.allocUtf8String(path))
  );
}
function runImportBatch(paths, kind) {
  try {
    const PHPhotoLibrary = frida_objc_bridge_default.classes.PHPhotoLibrary;
    if (!PHPhotoLibrary || !frida_objc_bridge_default.classes.PHAssetCreationRequest) {
      post("error", "PhotoKit classes not present in this process");
      return;
    }
    post("auth", PHPhotoLibrary.authorizationStatus());
      ...
      implementation: function() {
        for (let i = 0; i < paths.length; i++) {
          try {
            ...
            if (req === null) {
              post("error", "nil request for " + paths[i]);
              continue;
            }
...
function runImport(PATH) {
  runImportBatch([PATH], "video");
}
function appDirs() {
  const NSHomeDirectory = new NativeFunction(
    Module.getGlobalExportByName("NSHomeDirectory"),
    "pointer",
    []
  );

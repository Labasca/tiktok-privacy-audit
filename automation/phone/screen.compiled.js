📦
96504 /automation/phone/screen.js
60660 /automation/phone/screen.js.map
✄
...
function pngOfImage(img) {
  ...
  const data = new frida_objc_bridge_default.Object(UIImagePNGRepresentation(img));
...
rpc.exports = {
  ping: function() {
    let create = null;
    try {
      create = Module.getGlobalExportByName("_UICreateScreenUIImage");
    } catch (e) {
    }
    return {
      ok: true,
      create: !!create,
      uikit: !!frida_objc_bridge_default.classes.UIImage
    };
  },
  tapNorm: function(x, y) {
    ...
    const enqueue = frida_objc_bridge_default.classes.UIApplication.sharedApplication() && frida_objc_bridge_default.classes.UIApplication.sharedApplication()["_enqueueHIDEvent:"];
    frida_objc_bridge_default.schedule(frida_objc_bridge_default.mainQueue, function() {
      function fire2(touch, mask) {
        ...
        if (enqueue) {
          try {
            frida_objc_bridge_default.classes.UIApplication.sharedApplication()._enqueueHIDEvent_(parent);
          } catch (e) {
            dispatch(client, parent);
          }
        } else {
    ...
    return { scheduled: true, x, y, enqueue: !!enqueue };
  },
  snapshot: function() {
    ...
    const wrapped = new frida_objc_bridge_default.Object(img);

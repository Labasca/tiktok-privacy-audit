📦
93968 /automation/phone/wake.js
58712 /automation/phone/wake.js.map
✄
...
rpc.exports = {
  wake: function() {
    ...
    function tryCall(label, fn) {
      try {
        const r = fn();
        out.tried.push({ label, ok: true, result: String(r) });
        return true;
      } catch (e) {
        out.tried.push({ label, ok: false, error: String(e) });
        return false;
      }
    }
    const SBBacklight = frida_objc_bridge_default.classes.SBBacklightController;
    const SBLock = frida_objc_bridge_default.classes.SBLockScreenManager;
    const SBUserAgent = frida_objc_bridge_default.classes.SBUserAgent;
    function go() {
      out.classes = {
      ...
      if (SBBacklight) {
        ...
        if (inst) {
          ...
          tryCall("setBacklightFactor:source:", function() {
            inst["setBacklightFactor:source:"](1, 0);
            return "ok";
          });
        }
      }
    ...
    const t0 = Date.now();
    while (!done.v && Date.now() - t0 < 2e3) {
    }
    if (!done.v) go();
    out.ok = out.tried.some(function(t) {
      return t.ok;
    });
    return out;
  }

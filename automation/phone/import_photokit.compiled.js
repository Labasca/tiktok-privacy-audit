📦
95588 /automation/phone/import_photokit.js
60059 /automation/phone/import_photokit.js.map
✄
...
// automation/phone/import_photokit.js
function asString(v) {
...
function measureIdent(ident) {
  const PHAsset = frida_objc_bridge_default.classes.PHAsset;
  const PHAssetResource = frida_objc_bridge_default.classes.PHAssetResource;
  const arr = frida_objc_bridge_default.classes.NSArray.arrayWithObject_(ident);
  ...
  try {
    ...
    for (let i = 0; i < n; i++) {
      ...
      try {
        row.fileSize = Number(res.valueForKey_("fileSize"));
      } catch (e) {
      }
...
rpc.exports = {
  ping: function() {
    return {
      ok: true,
      photos: !!frida_objc_bridge_default.classes.PHPhotoLibrary,
      creation: !!frida_objc_bridge_default.classes.PHAssetCreationRequest
    };
  },
  importFile: function(path, kind) {
    ...
    if (!fm.fileExistsAtPath_(path)) {
      return { ok: false, error: "file not on phone: " + path, path };
    }
    try {
      if (type === 2) {
        ...
        const nsPath = frida_objc_bridge_default.classes.NSString.stringWithUTF8String_(
          Memory.allocUtf8String(path)
        );
        fn(nsPath, ptr(0), ptr(0), ptr(0));
      } else {
        const img = frida_objc_bridge_default.classes.UIImage.imageWithContentsOfFile_(path);
        if (!img || img.isNull()) {
          return { ok: false, error: "UIImage could not read " + path, path };
        }
        const fn = new NativeFunction(
      ...
      return { ok: false, error: "save threw: " + e, path, method: "uikit" };
    }
    return {
      ok: true,
      path,
